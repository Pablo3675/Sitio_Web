...
def gen_pass():
    ...
    for i in range(10):
        y= random.randint(0,len(caracteres))
        password += caracteres[y]

@app.route("/" )
def Hello():
    return "<h1>Hola, en esta página puedes aprender un  par de cosaas sobre las dependencias tecnológicas</h1> <a href='/random_fact'>¡Ver un dato aleatorio!</a>"
...
def secret_page():
    gen_pass()
    return f"<p>Contraseña aleatoria: {password}<p>"
